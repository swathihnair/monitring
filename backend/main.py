...
class ActivityDetector:
    def __init__(self):
        ...
        self.rapid_movement_threshold = 0.06  # Movement speed threshold - balanced
        ...
        self.movement_consistency_buffer = []  # Track consistent movement
    ...
    def detect_rapid_movement(self, landmarks):
        """Detect rapid movement based on position changes with consistency check"""
        if not landmarks:
            return False, 0.0
        ...
        # Calculate movement speed - only need 2 frames now
        if len(self.prev_positions) >= 2:
            ...
            is_rapid_single = movement > self.rapid_movement_threshold
            
            # Track consistency - need multiple consecutive rapid movements
            self.movement_consistency_buffer.append(is_rapid_single)
            if len(self.movement_consistency_buffer) > 5:
                self.movement_consistency_buffer.pop(0)
            
            # Only trigger alert if we have consistent rapid movement
            consistent_rapid_count = sum(self.movement_consistency_buffer)
            is_rapid = consistent_rapid_count >= self.movement_consistency_threshold
            
            # Log for debugging
            if movement > 0.02:  # Log any significant movement
                print(f"  Movement: {movement:.4f} (threshold: {self.rapid_movement_threshold:.4f}, consistent: {consistent_rapid_count}/{len(self.movement_consistency_buffer)}, alert: {is_rapid})")
    ...
    def detect_seizure(self, landmarks):
        """Detect seizure-like convulsive movements - must be ERRATIC, not just fast"""
        if not landmarks:
            return False, 0.0
        ...
        if len(self.prev_landmarks_history) < 8:
            return False, 0.0
        ...
        movement_max = np.max(movements)
        movement_std = np.std(movements)
        
        # Count rapid movements (spikes)
        rapid_movement_count = sum(1 for m in movements if m > 0.03)
        very_rapid_count = sum(1 for m in movements if m > 0.06)
        
        # SEIZURE = HIGH VARIANCE (erratic) + movement
        # Key difference: Seizures are ERRATIC (high variance), rapid movements are SMOOTH (low variance)
        is_seizure = (
            # High variance is KEY for seizure (erratic, not smooth)
            (movement_variance > 0.006 and movement_mean > 0.025) or  # Erratic + movement
            (movement_variance > 0.012) or  # Very erratic alone
            (movement_std > 0.05 and rapid_movement_count >= 4) or  # High inconsistency + spikes
            (very_rapid_count >= 3 and movement_variance > 0.004)  # Multiple very rapid + some variance
        )
        
        # Debug logging
        if movement_mean > 0.015 or movement_variance > 0.002:
            print(f"  Seizure check - Variance: {movement_variance:.4f}, Mean: {movement_mean:.4f}, Std: {movement_std:.4f}, Spikes: {rapid_movement_count}, Detected: {is_seizure}")
    ...
    def detect_bed_exit(self, landmarks, frame_shape):
        ...
        if self.bed_region is None:
            self.bed_region = {
                'x_min': hip_x - 0.25,  # Larger bed region
                'x_max': hip_x + 0.25,
                'y_min': hip_y - 0.25,
                'y_max': hip_y + 0.25
            }
        ...
        # Check if patient is outside bed region (with larger tolerance)
        is_outside = (
            hip_x < self.bed_region['x_min'] - 0.1 or  # Added tolerance
            hip_x > self.bed_region['x_max'] + 0.1 or
            hip_y < self.bed_region['y_min'] - 0.1 or 
            hip_y > self.bed_region['y_max'] + 0.1
        )
    ...
    def detect_breathing_rate(self, landmarks):
        ...
        # Classify breathing rate with more lenient ranges
        if breaths_per_minute < 10:
            status = "Slow (Bradypnea)"
        elif breaths_per_minute > 25:
    ...
    def analyze_frame(self, frame, frame_number=0):
        ...
        if detection_result.pose_landmarks:
            ...
            # 2. Detect seizure FIRST (highest priority)
            is_seizure, seizure_conf = self.detect_seizure(landmarks)
            # Apply cooldown to prevent duplicate alerts
            if is_seizure and (frame_number - self.last_seizure_frame) > self.seizure_cooldown:
                activities["seizure_detected"] = True
                self.last_seizure_frame = frame_number
            else:
                activities["seizure_detected"] = False
            activities["seizure_confidence"] = seizure_conf
            
            # 3. Detect rapid movement (but NOT if seizure is detected)
            is_rapid, speed = self.detect_rapid_movement(landmarks)
            # Only trigger rapid movement if NOT a seizure
            if activities["seizure_detected"]:
                activities["rapid_movement"] = False  # Suppress rapid movement during seizure
            else:
...
async def process_video(filename: str):
    ...
    try:
        ...
        while cap.isOpened():
            ...
            # Process every 3rd frame for better detection (changed from 5)
            if frame_count % 3 == 0:
                activities, _ = detector.analyze_frame(frame, frame_count)
                
                timestamp = frame_count / fps
                
                # Debug logging
                if activities["pose_detected"]:
                    print(f"Frame {frame_count}: Pose detected, Movement speed: {activities['movement_speed']:.4f}, Rapid: {activities['rapid_movement']}")
                else:
                ...
                # Monitor breathing rate (alert if abnormal) - more lenient thresholds
                if activities["breathing_rate"] > 0:
                    if activities["breathing_rate"] < 8 or activities["breathing_rate"] > 30:
                        alert = {
